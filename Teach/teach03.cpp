...
*
* Instructions:
* As a team, write a program that reads a list of scriptures from a file
* and displays them. Use the stringstream library to process and handle 
* lines that have multiple verses ("Moroni 10:3-5" in addition to simply 
* "Moroni 10:4").
************************************************************************/
#include <iomanip>
#include <iostream>
 ...
 ***********************************************************************/
int readFile(std::string &file, std::string scriptureFile[])
{
	...
	while (!fin.eof())
	{
		std::getline(fin, scriptureFile[i]);
		i++;
	}
	//close file
	fin.close();
...
* function.
 ***********************************************************************/
void parseStream(std::string scriptureFile[], int size, Scripture scripture[])
{
...
void display(int size, Scripture scripture[])
{
	for (int i(0); i < size; i++)
	{
		...
		//at all (e.g., "Alma 31").
		std::cout << scripture[i].book << " " << scripture[i].chapter;
		...
		if(scripture[i].startVerse > scripture[i].endVerse && 
			...
			std::cout << ":" << scripture[i].endVerse << "-" 
			<< scripture[i].startVerse << std::endl;
		else
		{
			if (scripture[i].startVerse)
			{
				std::cout << ":" << scripture[i].startVerse;
			}
			...
			{
				std::cout << "-" << scripture[i].endVerse << std::endl;
			}
			else
				std::cout << std::endl;
		}
...
int main()
{
	std::string file = promptFile();
	std::string scriptureFile[50] = {};
